...
describe("parseCsvLine", () => {
    it("parses a simple comma-separated line", () => {
        expect((0, csvParser_1.parseCsvLine)("a,b,c")).toEqual(["a", "b", "c"]);
    });
    it("parses a single field with no commas", () => {
        expect((0, csvParser_1.parseCsvLine)("hello")).toEqual(["hello"]);
    });
    it("returns an empty string for an empty input", () => {
        expect((0, csvParser_1.parseCsvLine)("")).toEqual([""]);
    });
    it("handles empty fields", () => {
        expect((0, csvParser_1.parseCsvLine)("a,,c")).toEqual(["a", "", "c"]);
    });
    it("handles a trailing comma producing an empty last field", () => {
        expect((0, csvParser_1.parseCsvLine)("a,b,")).toEqual(["a", "b", ""]);
    });
    it("handles a leading comma producing an empty first field", () => {
        expect((0, csvParser_1.parseCsvLine)(",b,c")).toEqual(["", "b", "c"]);
    });
    it("strips surrounding double-quotes from a field", () => {
        expect((0, csvParser_1.parseCsvLine)('"hello"')).toEqual(["hello"]);
    });
    it("allows commas inside a quoted field", () => {
        expect((0, csvParser_1.parseCsvLine)('"hello, world",foo')).toEqual(["hello, world", "foo"]);
    });
    it("handles escaped double-quotes (RFC 4180 style) inside a quoted field", () => {
        expect((0, csvParser_1.parseCsvLine)('"say ""hi"""')).toEqual(['say "hi"']);
    });
    it("handles multiple quoted fields", () => {
        expect((0, csvParser_1.parseCsvLine)('"a,b","c,d"')).toEqual(["a,b", "c,d"]);
    });
    it("handles a realistic CSV data row", () => {
        ...
        expect((0, csvParser_1.parseCsvLine)(line)).toEqual([
...
describe("parseCsvContent", () => {
    ...
    it("parses a single data row correctly", () => {
        ...
        const rows = (0, csvParser_1.parseCsvContent)(content);
